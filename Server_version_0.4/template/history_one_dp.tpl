...
<body>
<h2 id="t4">{{dp_name}} INFORMATION</h2>
...
<table id="history_one_ts_table_header">
  ...
  <tr>
    <th colspan = "2">DP ID</th>
    <th colspan = "3">{{DP_ID}}</th>
  </tr>
  <tr>
    %for r in rows:
      <th>{{r}}</th>
...
<table id="history_one_ts_table">
  %for i in range(cases):
     <tr>
      %for r in rows:
		%if r == "S. No":
			<td width="140px";>{{rows[r][i]}}</td>

		%elif r == "Error Code":
			<td width="265px";>{{rows[r][i]}} </td>
			
		

		%elif r == "Date":
			<td width="125px";>{{rows[r][i]}}</td>
		%elif r == "Time":
